  ...
  loader: async () => {
    try {
      const [rawProjects = [], rawTestimonials = []] = await Promise.all([
        getProjectsFn().catch((err) => {
          console.error("Failed to fetch projects from DB, using empty fallback:", err);
          return [];
        }),
        getTestimonialsFn().catch((err) => {
          console.error("Failed to fetch testimonials from DB, using empty fallback:", err);
          return [];
        }),
      ]);
      ...
      const projects = (rawProjects || [])
        ...
        .map((p) => ({
          ...p,
          industry: p.industry_tag || "Automation",
          service: p.service_tag || "AI Engineering",
          summary: p.context_body ? (p.context_body.length > 150 ? p.context_body.slice(0, 150) + "..." : p.context_body) : (p.seo_description || p.title || ""),
          roi: [
