...
const process = [
  { step: "01", title: "Intro call", body: "30 minutes with the hiring manager on scope, motivation, and mutual fit." },
  { step: "02", title: "Work sample", body: "A real, scoped problem from our engagements — paid if it takes more than a couple of hours." },
  { step: "03", title: "Team interviews", body: "Two conversations: one technical, one on judgment and communication." },
  { step: "04", title: "Reference & offer", body: "Two references, then a clear offer within a week — no black holes." },
];
        ...
        <Container size="wide" className="relative py-16 sm:py-24">
          <Reveal>
            ...
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-muted-foreground">
              We are a small, deliberately unbloated team building the AI and automation systems clients actually
              run their business on. If that sounds better than another layer of process, read on.
            </p>
          </Reveal>
        </Container>
      </div>
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2">
            {careerValues.map((v, i) => (
              <Reveal key={v.title} delay={i * 0.06} className="rounded-3xl border border-hairline bg-card p-7">
                ...
                <p className="mt-3 text-[0.9375rem] leading-relaxed text-muted-foreground">{v.body}</p>
              </Reveal>
            ))}
          </div>

          <Reveal className="mt-4 rounded-3xl border border-hairline bg-secondary/50 p-7 sm:p-9">
            <p className="font-mono text-[0.7rem] tracking-widest text-muted-foreground uppercase">Benefits</p>
      ...
      <Section>
        <Container size="wide">
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {process.map((p, i) => (
              <Reveal key={p.step} delay={i * 0.06} className="rounded-3xl border border-hairline bg-card p-6">
